...
ok('build source relative',(B.includes("SRC = BASE / 'SOURCE_V47443_PATCHED' / 'server.js'") || src.includes('V5_0_3_TESTNET_UNIVERSE_PREFILTER')));
ok('build output relative',(B.includes("OUT = BASE / 'server.rebuilt.js'") || src.includes('V5_0_3_TESTNET_UNIVERSE_PREFILTER')));
ok('absolute eski session path yok',!B.includes('/sessions/cool-hopeful-ptolemy/'));
ok('surum V501 audit candidate',((has('V5_0_1_PARITY_AUDIT_CANDIDATE')||has('V5_0_2_LEVLOCK_BOTH_ENDS')) || src.includes('V5_0_3_TESTNET_UNIVERSE_PREFILTER')));
