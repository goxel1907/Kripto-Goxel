...
const LAZARUS_BUILD = 'R284_BOZUK_ZEMIN_GARDI';
  ...
  try {
    ...
    // ── İKİ KATMANLI KARAR MEKANİZMASI ─────────────────────────────────────────
      ...
      function evalDecision(side){
        ...
        // R284: BOZUK ZEMİN + OYNAKLIK GARDI — VELVET LONG -6.40 dersi.
        // R160 4/4 "tam kanıt" olsa bile, zemin BOZUK + ATR oynak ise giriş tuzaktır
        // (VELVET: %13.3 ATR, BOZUK zemin, düşen bıçak → -12.7%). Sağlam zeminli 4/4'lere
        // (HOME/ENA/STG kazananları) dokunmaz; sadece BOZUK+oynak kombinasyonu keser.
        const r284GroundBroken = /BOZUK|TEHLİKELİ/i.test(String(d.r93PiyasaEtiketi||''));
        const r284Volatile = (atrPct >= 6.0);
        const r284BrokenGroundBlock = !!(r284GroundBroken && r284Volatile);
        const hardVeto = !!(poorLiquidity || atrBlocking || r39TargetNearBlock || r39TargetNearHardBlock || r41TrapBlock || r41FallingKnifeBlock || r41RisingKnifeBlock || !fundOk || !rsiOk || !mmOk || r29CtxBlock || r196RangeBlock || r284BrokenGroundBlock);
        const hardVetoReasons=[];
        if(r284BrokenGroundBlock) hardVetoReasons.push(`R284 BOZUK zemin + oynak (ATR %${atrPct.toFixed(1)}) — 4/4 olsa bile tuzak (VELVET tipi)`);
